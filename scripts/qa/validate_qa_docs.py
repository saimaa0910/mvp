...
"""
validate_qa_docs.py
Comprehensive quality gate validator for Namma Clinic Phase 11 (QA Engineering Planning & Test Design).

Enforces 8 comprehensive quality, architectural, and integrity gates:
1. Document Presence (20 authoritative documents in docs/11-qa/)
2. Line Count Mandate (>= 2,000 SUBSTANTIVE lines per document, counted via count_lines())
3. Canonical Registry Integrity & Uniqueness (20 registries, 2,086 items)
...
7. Documentation-First Policy (Zero runtime application code, pure test designs, DOCUMENTATION-ONLY tagged code)
8. Upstream Document Preservation (docs/00- to docs/10- untouched)
...
REQUIRED_QA_DOCS = [
    ...
    "06-e2e-test-plan.md",
    "07-ui-test-plan.md",
    "08-performance-test-plan.md",
    "09-security-test-plan.md",
    "10-offline-test-plan.md",
    "11-data-quality-test-plan.md",
    "12-accessibility-test-plan.md",
    "13-localization-test-plan.md",
    "14-regression-strategy.md",
    "15-uat-plan.md",
    "16-pilot-test-plan.md",
    "17-test-data-strategy.md",
    "18-test-environment.md",
    "19-quality-gates.md",
    "QA_COMPLETENESS_AUDIT.md",
]
...
def main():
    print("=" * 95)
    print("NAMMA CLINIC DIGITAL HEALTH PLATFORM - PHASE 11 QA MASTER QUALITY GATE")
    ...
    for fname in REQUIRED_QA_DOCS:
        fpath = QA_DOCS_DIR / fname
        if not fpath.exists():
    ...
    if missing_docs:
        ...
        print("  [PASS] All 20 required Phase 11 documents present on disk.")
    ...
    print(f"\n  Phase 11 Volume Summary: {total_substantive:,} substantive lines ({total_lines:,} total lines)")
    ...
    registry_checks = [
        ("Test Strategies", len(TEST_STRATEGIES), 20),
        ("Test Levels Hierarchy", len(TEST_LEVELS), 15),
        ("Detailed Test Cases", len(TEST_CASES), 1000),
        ("E2E Clinical Scenarios", len(TEST_SCENARIOS), 50),
        ("Synthetic Test Datasets", len(TEST_DATASETS), 50),
        ("Defect Taxonomy Rules", len(DEFECT_REGISTRY), 40),
        ("Release Quality Gates", len(QUALITY_GATES), 30),
        ("Performance Benchmarks", len(PERFORMANCE_TESTS), 50),
        ("Security Quality Tests", len(SECURITY_TESTS_QA), 60),
        ("Offline Resilience Tests", len(OFFLINE_TESTS), 50),
        ("Accessibility Checks", len(ACCESSIBILITY_TESTS), 50),
        ("Localization Checks", len(LOCALIZATION_TESTS), 50),
        ("API Route Test Cases", len(API_TESTS), 60),
        ("Database Invariant Tests", len(DATABASE_TESTS), 50),
        ("UI Component Tests", len(UI_TESTS), 60),
        ("Integration Boundary Tests", len(INTEGRATION_TESTS), 50),
        ("Clinician UAT Scenarios", len(UAT_TESTS), 40),
        ("Clinic Pilot Field Tests", len(PILOT_TESTS), 30),
        ("Regression Test Suites", len(REGRESSION_SUITES), 25),
        ("Environment Topologies", len(ENVIRONMENT_CONFIGS), 15),
    ]
    ...
    print(f"\n  Total Canonical QA Registry Entities: {total_registry_items:,} unique items")
    if registry_failures:
        gate_results.append(("GATE 3: Registry Counts", False, f"{len(registry_failures)} registry failures"))
        all_passed = False
    else:
        print("  [PASS] All 20 canonical QA registries meet 100% of count expectations.")
    ...
    trace_errors = []
    audit_doc = doc_contents.get("QA_COMPLETENESS_AUDIT.md", "")
    ...
    runtime_files = [f for f in QA_DOCS_DIR.glob("*") if f.suffix in [".ts", ".tsx", ".js", ".jsx", ".py"]]

    if runtime_files:
        print(f"  [FAIL] Found runtime code files in docs/11-qa/: {runtime_files}")
        gate_results.append(("GATE 7: Doc-First Policy", False, f"{len(runtime_files)} runtime files"))
        all_passed = False
    else:
        print("  [PASS] Zero runtime application code files in docs/11-qa/; pure documentation specifications.")
        gate_results.append(("GATE 7: Doc-First Policy", True, "100% compliant"))

    # -------------------------------------------------------------------------
    # GATE 8: Upstream Document Preservation
    # -------------------------------------------------------------------------
    print("\n[GATE 8] Upstream Phase Preservation Verification (docs/00- to docs/10-)...")
    upstream_dirs = [
        "00-project-baseline",
    ...
    if missing_upstream:
        ...
        print(f"  [PASS] All 11 upstream documentation phases (docs/00- to docs/10-) intact.")
        gate_results.append(("GATE 8: Upstream Preservation", True, "All 11 phases intact"))

    # -------------------------------------------------------------------------
    # SUMMARY
    # -------------------------------------------------------------------------
    print("\n" + "=" * 95)
    print("PHASE 11 QA QUALITY GATE SUMMARY REPORT")
    print("=" * 95)
    ...
    if all_passed:
        print("\n>>> OVERALL RESULT: 100% PASS - PHASE 11 QA ENGINEERING BASELINE APPROVED <<<\n")
        return 0
    else:
